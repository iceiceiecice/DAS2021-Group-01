...
print(pairplot + scale_colour_manual(values=c("lightskyblue3", "blue1"))+ scale_fill_manual(values=c("lightskyblue1", "blue1")))
```

Table 2 shows the summaries for each of the numerical variables. There is no missing data within these variables and so no values will need to be imputed for the analysis in the report. There appear to be possible outliers at the maximum values of Total Household Income and House Floor Area.
...
plot_model(model_sex_number, show.values = TRUE, transform = NULL,
           title = "Log-Odds (Male household)", show.p = FALSE, col = "royalblue1")
```
...
plot_model(model_sex_number, show.values = TRUE, axis.lim = c(1,1.5),
           title = "Odds (Male household)", show.p = FALSE, col="royalblue1")
```
...
Table shows the estimates and the lower and upper bounds of the 95% confidence intervals for the regression parameters. The rows containing significant predictors, and so where the confidence intervals do not include 0, are highlighted. 

```{r table of estimates and confidence intervals}
...
```{r table of estimates and confidence intervals 2}
...
poisson.sig.int%>% kable() %>%
  kable_styling(position = "center", full_width = FALSE) %>%
  row_spec(c(1,2,3,4,5,6,7), background  = "lightblue", color="black")
```
