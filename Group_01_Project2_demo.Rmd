...
The scatterplot in Figure 9 of total household income against total food expenditure suggests a moderate positive correlation but the fitted model may be being heavily influenced by the extreme values, particularly by one extreme point.
...
where
 ...
 - $log(\tilde{\lambda_i})$ is the Total Number of Household Members of the $i^{th}$ household;
...
Figure 16 shows the observed number of household members against the expected number of household members, there is a definite moderate positive correlation between these variables but it is not a perfect correlation. This suggests the model may be improved upon, which is an area for further analysis.
